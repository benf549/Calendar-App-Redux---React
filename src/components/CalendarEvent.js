...
let CalendarEvent = ({
	...
	//runs when the mouse enters an event object on the screen. Sets the height2 variable equal to the height of the eventtitle object.
	const handleenter = () => {
	...
	useEffect(() => {
		const Testt = {
			"--hovered-element-height": `${
				height2 > totalheight * 0.01 * height
					? height2
					: totalheight * 0.01 * height
			}px`,
		};
		document.documentElement.style.setProperty(
			"--hovered-element-height",
			Testt["--hovered-element-height"]
		);
